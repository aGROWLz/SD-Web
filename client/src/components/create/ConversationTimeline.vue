    ...
    <article v-for="entry in entries" :key="entry.id" class="task-row">
      <section class="task-config-card">
        ...
        <div class="task-times">
          ...
          <time v-if="entry.createdAt && entry.completedAt">用时 {{ durationSeconds(entry.createdAt, entry.completedAt) }} 秒</time>
        </div>

        <div v-if="entry.snapshot.prompt" class="prompt-block">
          <p class="task-prompt truncated">{{ entry.snapshot.prompt }}</p>
          <button v-if="isLongPrompt(entry.snapshot.prompt)" type="button" class="prompt-expand" :aria-expanded="expandedEntryId === entry.id" @click="togglePrompt(entry.id)">
            <span>{{ expandedEntryId === entry.id ? '收起提示词' : '展开完整提示词' }}</span><el-icon :class="{ rotated: expandedEntryId === entry.id }"><ArrowDown /></el-icon>
          </button>
          <div v-if="expandedEntryId === entry.id" class="prompt-expanded-panel">{{ entry.snapshot.prompt }}</div>
        </div>
...
import { ArrowDown, CircleCheckFilled, Edit, Loading, MagicStick, RefreshRight, WarningFilled } from '@element-plus/icons-vue'
...
// Keep the control visible across browsers even when the prompt wraps differently.
const isLongPrompt = (prompt: string) => prompt.trim().length > 30
const togglePrompt = (entryId: string) => {
...
const durationSeconds = (createdAt: string, completedAt: string) => {
  const seconds = Math.max(0, Math.round((new Date(completedAt).getTime() - new Date(createdAt).getTime()) / 1000))
  return seconds
}
</script>

<style scoped>
.task-row{content-visibility:auto;contain-intrinsic-size:320px}
...
.task-config-card{position:relative}.prompt-block{position:relative;min-width:0}.task-prompt.truncated{display:-webkit-box;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:1;white-space:nowrap;text-overflow:ellipsis}.prompt-expand{display:inline-flex;align-items:center;gap:3px;margin-top:5px;padding:0;border:0;color:var(--accent-primary);background:transparent;font-size:11px;cursor:pointer}.prompt-expand .el-icon{transition:transform .18s ease}.prompt-expand .el-icon.rotated{transform:rotate(180deg)}.prompt-expanded-panel{position:absolute;z-index:8;top:calc(100% + 8px);right:0;left:0;max-height:240px;overflow:auto;padding:12px 14px;border:1px solid var(--border-emphasis);border-radius:6px;color:var(--text-primary);background:var(--bg-elevated);box-shadow:0 14px 34px rgba(0,0,0,.42);font-size:13px;line-height:1.65;white-space:pre-wrap;overflow-wrap:anywhere}
</style>
